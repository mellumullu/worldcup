...
echo "$($PSQL "SELECT DISTINCT t.name FROM games g JOIN teams t ON g.winner_id = t.team_id JOIN teams t2 ON g.opponent_id = t2.team_id WHERE g.round = 'Eighth-Final' AND g.year = 2014 UNION SELECT DISTINCT t2.name FROM games g JOIN teams t ON g.winner_id = t.team_id JOIN teams t2 ON g.opponent_id = t2.team_id WHERE g.round = 'Eighth-Final' AND g.year = 2014")"
